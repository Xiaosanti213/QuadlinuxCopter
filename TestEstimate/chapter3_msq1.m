...
H = zeros(1,dim,sim_finish+1);

P = zeros(dim,dim,sim_finish+1);%初始估计的协方差阵
P(:,:,1) = eye(2);

y = zeros(1,1,sim_finish+1);
...
subplot(211)
plot(1:1:sim_finish+1,temp_x(1,:),'r-');
hold on
plot(1:1:sim_finish+1,temp_x(2,:),'b.');
xlabel('simu_times')
ylabel('估计值')
...
subplot(212)
plot(1:1:sim_finish+1,temp_p(1,:),'r-');
hold on 
plot(1:1:sim_finish+1,temp_p(2,:),'b.');
xlabel('simu_times')
ylabel('估计协方差')
