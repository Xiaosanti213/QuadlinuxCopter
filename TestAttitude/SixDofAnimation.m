function fig = SixDOFanimation(varargin)
    ...
    %% Create local variables  产生变量

    % Required arguments 这两个是必须需要的下面是可以使用缺省的
    p = varargin{1};                % position of body  第一个自变量是位置
    R = varargin{2};                % rotation matrix of body 旋转矩阵
    [numSamples dummy] = size(p);   % 

    % Default values of optional arguments  一些缺省值
    SamplePlotFreq = 1;
    Trail = 'Off';
    LimitRatio = 1;
    Position = [];
    ...
    for i = 3:2:nargin %一个一个测试输入的参数的名称（隔一个是为了跳过参数）
        ...
        elseif  strcmp(varargin{i}, 'Position'), Position = varargin{i+1};%默认的是[9 39 1280 768]
    ...
    % Create plot data arrays 建立绘图数据的矢量 
    ...
    if(strcmp(Trail, 'All'))
        ...
        oz = zeros(numPlotSamples, 1);%需要三维坐标点一个
        ...
        wz = zeros(numPlotSamples, 1);%需要三维矢量三根
    end
    x(1) = p(1,1);
    y(1) = p(1,2);
    z(1) = p(1,3);
